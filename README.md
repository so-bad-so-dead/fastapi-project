...
- Инструкция по запуску (Docker и локально)
- Описание схемы БД
...
Примечание: проект использует SQLAlchemy `Base.metadata.create_all()` для создания таблиц на старте (в `app/tasks.py`). Для продакшена рекомендуется использовать Alembic миграции.